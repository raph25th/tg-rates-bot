...
- Уведомления после обновления курса ЦБ РФ.
...
Пользователь может включить уведомления в разделе `🔔 Уведомления ЦБ`.

Бот проверяет обновление курса ЦБ РФ вечером по timezone `Europe/Moscow` и отправляет сообщение после публикации нового курса. Уведомление отправляется пользователю только один раз за новую дату курса: дата последней отправки хранится в `last_sent_cbr_date`.

Если ЦБ РФ или Telegram временно недоступны, бот логирует ошибку и попробует снова на следующей проверке.
